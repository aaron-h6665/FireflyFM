...
struct ChatDailyActivityComposerView: View {
    ...
    private let client = CareClient.live
    ...
    private func loadChild() async {
        ...
        do {
            child = try await client.fetchChildren(schoolId)
...
struct CareTodayView: View {
    ...
    @EnvironmentObject private var appSession: AppSessionManager
    @State private var model = CareTodayModel()
    ...
    private var accessPolicy: CareAccessPolicy {
        CareAccessPolicy(context: appSession.accessContext())
    }

    private var childrenById: [UUID: Child] { Dictionary(uniqueKeysWithValues: model.children.map { ($0.id, $0) }) }
    ...
    var body: some View {
        ZStack(alignment: .bottomTrailing) {
            ...
            if model.children.isEmpty == false && accessPolicy.canRecord {
                Menu {
                    ForEach(model.children) { child in Button(child.fullName) { composingChild = child } }
                } label: {
    ...
    @MainActor private func load() async {
        ...
        await model.load(scope: CareTodayScope(
        ...
        if opensComposer, !didOpenInitialComposer,
           let initialChildId,
           let child = model.children.first(where: { $0.id == initialChildId }) {
...
private struct CareEventComposerView: View {
    ...
    @MainActor private func loadMedicationTasks() async {
        do {
            medicationTasks = try await client.fetchMedicationTasks(child.schoolId, child.id)
        ...
        Task {
            ...
            do {
                if !staffOnly, let roomId, let mediaData, let mediaContentType, let mediaFileName {
                    uploadedMedia = try await client.uploadMedia(mediaData, mediaFileName, mediaContentType, child.schoolId, roomId)
                    if let uploadedMedia {
                        sentMediaMessage = try await client.sendMediaMessage(roomId, uploadedMedia)
                    }
                }
                _ = try await client.recordEvent(CareEventRecordRequest(
                    childId: child.id,
                    type: eventType,
                    occurredAt: occurredAt,
                    details: details,
                    isStaffOnly: staffOnly,
                    medicationTaskId: selectedMedicationTaskId,
                ...
                if let sentMediaMessage {
                    try? await client.deleteMessage(sentMediaMessage.id)
                } else if let uploadedMedia {
                    try? await client.removePrivateFiles([uploadedMedia.path])
                }
...
private struct CareEventPhoto: View {
    ...
    var body: some View {
        ...
        .clipShape(RoundedRectangle(cornerRadius: 12))
        .task { url = try? await client.signedMediaURL(path) }
    }
}
...
struct ChatActivityLabelView: View {
    ...
    private func loadExistingEvent() async {
        ...
        isLoading = true
        do {
            let event = try await client.fetchEvent(eventId)
    ...
    private func save() {
        ...
        errorMessage = nil
        let trimmedSummary = summary.trimmingCharacters(in: .whitespacesAndNewlines)
        let request = CareActivityLabelRequest(
        ...
        Task {
            do {
                ...
                if let eventId = message.linkedCareEventId {
                    saved = try await client.correctActivity(eventId, request)
                } else {
                    saved = try await client.labelMessage(request)
                }
                await MainActor.run {
                    isSaving = false
                    onSaved(saved)
