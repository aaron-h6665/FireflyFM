...
| Google Forms body read | Form title, structure, questions, and responder link for Forms the account can access | Let the director select a Form and configure its onboarding fields |
...
Contacts, or unrelated Drive files. It does not request write, delete, or
sharing access to Google Forms or Drive. The Service uses Google user data only
to provide or improve the visible, user-facing Form-configuration and
onboarding features described above.
